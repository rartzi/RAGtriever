...
## 1) Product summary
**Mneme Local** is a local-first daemon + library that continuously indexes a changing filesystem "vault" (Obsidian-compatible Markdown + attachments) into a hybrid retrieval system (semantic + keyword + link-graph) and serves results to coding agents via MCP and Python.
